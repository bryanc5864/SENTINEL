#!/usr/bin/env python3
"""ToxiGene v8 — Multi-task SimpleMLP on v3 expanded dataset (2500 samples).

Builds on ToxiGene v7 (best-performing architecture) and trains on the
v3-expanded dataset which adds 708 real zebrafish samples from 16 new GEO
datasets plus 95 class-conditional augmented samples (total 2500 vs 1697).

Changes from v7:
  1. Uses v3 data: expression_matrix_v3_expanded.npy (2500, 61479)
  2. Proportional 70/15/15 split: 1750 train / 375 val / 375 test
  3. Slightly larger hidden layers (512→512, 256→256) to match data scale
  4. Reduced dropout (0.3→0.25) — more data allows less regularization
  5. Longer patience (50→60) — more samples = slower convergence

Architecture (ToxiGene v8):
  61479 genes
  → Linear(61479, 512) → BN → ReLU → Dropout(0.25)
  → Linear(512, 256)   → BN → ReLU → Dropout(0.25)   ← shared repr
...
CKPT_PATH    = CKPT_DIR / "toxigene_v8_best.pt"
RESULTS_PATH = CKPT_DIR / "results_v8.json"
LOG_PATH     = LOG_DIR  / "train_toxigene_v8.log"

# ── Hyperparameters ───────────────────────────────────────────────────────────
HIDDEN1        = 512
HIDDEN2        = 256
DROPOUT        = 0.25     # reduced from 0.3 (more data, less regularization needed)
N_PATHWAY      = 200
...
WEIGHT_DECAY  = 0.01
GRAD_CLIP     = 1.0
EARLY_STOP    = 60        # increased from 50 (more samples per epoch)
SEED          = 42

# Minimal augmentation (same as v7)
GENE_DROP_RATE = 0.10
NOISE_PROB     = 0.40
NOISE_STD      = 0.01

# 70/15/15 split for N=2540
N_TRAIN = 1778
N_VAL   = 381
N_TEST  = 381
...
def main() -> None:
    ...
    # ── Load data ────────────────────────────────────────────────────────────
    log.info("Loading expression + outcome + pathway data (v3) …")
    X = np.load(str(DATA_DIR / "expression_matrix_v3_expanded.npy"))   # (2500, 61479)
    y = np.load(str(DATA_DIR / "outcome_labels_v3_expanded.npy")).astype(np.float32)
    p = np.load(str(DATA_DIR / "pathway_labels_v3_expanded.npy")).astype(np.float32)
    ...
    n_total = len(X)
    log.info(f"Split: {N_TRAIN} / {N_VAL} / {n_total - N_TRAIN - N_VAL} (total={n_total})")
    ...
    # ── Build model ───────────────────────────────────────────────────────────
    log.info("\n" + "=" * 60)
    log.info("Building ToxiGene v8 (multi-task SimpleMLP, v3 data) …")
    model = ToxiGeneV7(
    ...
    log.info("\n" + "=" * 60)
    log.info("TEST RESULTS: ToxiGene v8")
    log.info("=" * 60)
    ...
    results = {
        ...
        "toxigene_v7_f1": 0.8860,
        "baseline_simpleMLP_f1": 0.8896,
        "delta_vs_v7": round(test_m["f1"] - 0.8860, 4),
        "delta_vs_baseline": round(test_m["f1"] - 0.8896, 4),
        "n_train": N_TRAIN,
        "n_val": N_VAL,
        "n_test": n_total - N_TRAIN - N_VAL,
        "n_total": n_total,
        "n_new_geo_samples": 843,
    ...
    print(
        f"ToxiGene v8 TEST F1: {test_m['f1']:.4f}  "
        f"(v7: 0.8860, baseline: 0.8896, delta_v7: {test_m['f1']-0.8860:+.4f})"
    )
