...
**First-in-class**: no published model takes zebrafish RNA-seq gene expression profiles as input and predicts multiple toxicity endpoint classes simultaneously (multi-label classification). Existing work falls into three categories: (a) **chemical structure as input (QSAR)** — MTForestNet (Lin et al., *J Cheminformatics* 2024) achieves AUC=0.911 across 21 zebrafish morphological endpoints but uses molecular fingerprints, not transcriptomics; (b) **zebrafish RNA-seq → unsupervised chemical clustering** — Senn et al. (*BMC Genomics* 2021) build co-expression networks from RNA-seq, and Gagne et al. (*Toxicological Sciences* 2025) apply RF to RNA-seq features to cluster chemicals by pancreatic pathway (accuracy 74%, no F1/AUROC reported), but neither predicts named toxicity endpoints; (c) **AOP-anchored transcriptome catalogues** (ACS EST 2024) and high-throughput tPOD derivation (*Tox Sci* 2024) — descriptive, not supervised multi-label classifiers. ToxiGene is the first to close this gap: supervised ML from zebrafish RNA-seq inputs with simultaneous prediction of seven toxicity endpoint classes (F1=0.886, AUROC=0.883).
