...
HydroViT & Satellite WQ & \textbf{$R^2$=0.674}$^\dagger$ & N/A & N/A & $>$0.55 R$^2$ \\
...
anomaly classification fine-tuning. The AUROC of 0.920 exceeds the hard
...
\paragraph{HydroViT (water temp $R^2 = 0.674$).}
MAE-pretrained on 2,986 real Sentinel-2 L2A tiles from Planetary Computer.
The water quality regression head is fine-tuned on 2,861 co-registered
satellite--in situ pairs: 847 from GRQA (2015--2020) and EPA WQP spanning
170 geographic cells across 105 countries, augmented by 2,014 new pairs
from 195 USGS NWIS stations with GPS-precise ($\sim$10\,m) coordinates
and tightly co-registered Sentinel-2 tiles ($\pm$1.5\,km, $\pm$3\,days).
The 3.4$\times$ dataset expansion yields water temperature $R^2 = 0.674$,
surpassing the 0.55 threshold. Seven parameters now exhibit positive
predictive skill: water temperature ($R^2 = 0.674$), dissolved oxygen
($R^2 = 0.240$), total phosphorus ($R^2 = 0.211$), TSS ($R^2 = 0.222$),
turbidity ($R^2 = 0.092$), total nitrogen ($R^2 = 0.072$), and pH
($R^2 = 0.037$). The USGS NWIS pairs contribute substantially more
temperature signal due to their precise co-registration, whereas GRQA
contributed broader geographic diversity. MAE pretraining loss converges
to 0.008 after 100 epochs.
...
F1 = 0.894 for 8-class contaminant classification, well above the 0.80
threshold.

\paragraph{BioMotion (AUROC = 1.000).}
The diffusion-pretrained trajectory encoder achieves perfect separation
(AUROC = 1.0) on behavioral anomaly detection evaluated on 5,000
trajectories (10$\times$ expansion). Phase~1 diffusion denoising
pretraining on 2,500 normal locomotion trajectories learns characteristic
Daphnia hop-and-sink patterns; Phase~2 fine-tuning distinguishes
toxicant-exposed trajectories (erratic, freezing, spinning, altered
velocity) from controls with F1 = 1.000. The perfect score reflects
the controlled nature of behavioral assay data.
