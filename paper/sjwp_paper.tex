...
\caption{\textbf{Left}: AUROC drop when each modality is removed---sensors and behavioral monitoring are most critical. \textbf{Right}: Cumulative gain from best single (0.943) to full 5-modal fusion (0.992, $p = 0.002$).}
\label{fig:ablation}
\end{figure}
...
\caption{\textbf{Left}: Alert rate at threshold 0.9---zero false positives across 10 clean NEON sites (8,739 windows), while contamination events trigger 10--100\% detection. \textbf{Right}: Temporal persistence---real events sustain up to 89 consecutive above-threshold windows vs.\ 0 for clean sites (ratio 31:0).}
\label{fig:fpr}
\end{figure}
