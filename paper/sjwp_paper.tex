...
\caption{SENTINEL encoders vs.\ published state-of-the-art and baselines on standardized benchmark splits. Dark green = SENTINEL (ours); navy = published SOTA with citations; blue = ML/DL baselines; grey hatched = external methods on different tasks (not directly comparable). Dashed red lines mark performance thresholds. AquaSSM outperforms MCN-LSTM [24] by +0.052 AUROC on USGS sensor data ($n = 762$). HydroViT v9 outperforms DenseNet121/HydroVision [23] by +0.009 R$^2$ on water temperature ($n = 5{,}464$). BioMotion outperforms the Deep Autoencoder [25] by +0.042 AUROC on behavioral data ($n = 28{,}610$). MicroBiomeNet establishes a first-in-class benchmark for 8-class EMP aquatic source classification. ToxiGene is the first supervised method for multi-label toxicity classification from RNA-seq---closest prior art either uses molecular fingerprints as input (MTForestNet [26], AUC = 0.911 on a different task) or performs unsupervised clustering without classification metrics [27, 28].}
\label{fig:sota}
\end{figure}
