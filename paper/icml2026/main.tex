...
Table~\ref{tab:db} summarises the data composition by source and modality.
...
Per-encoder benchmarks (Figure~\ref{fig:bench}, Table~\ref{tab:encoders}) place each branch ahead of the strongest published single-stream baseline on its own held-out test set: AquaSSM at AUROC $0.916$ ($+0.052$ over MCN-LSTM); HydroViT v9 at $R^2\!=\!0.893$ on water temperature ($+0.009$ over DenseNet121); BioMotion at AUROC $1.000$ at the assay level ($+0.042$ over a Deep Autoencoder); MicroBiomeNet at macro-F$_1\!=\!0.899$ ($-0.006$ vs.\ tuned SimpleMLP); and ToxiGene at $7$-class macro-F$_1\!=\!0.877$ as a first-in-class supervised method. Bootstrap $95\%$ confidence intervals are reported in Table~\ref{tab:encoders}.
...
\begin{table*}[t]
\caption{Per-encoder benchmark results with $95\%$ confidence intervals and strongest baselines. Metrics: AUROC for AquaSSM and BioMotion; $R^2$ for HydroViT; macro-F$_1$ for MicroBiomeNet and ToxiGene.}
\label{tab:encoders}
\centering
\small
\begin{tabular}{llcccc}
\toprule
\textbf{Encoder} & \textbf{Metric} & \textbf{SENTINEL} & \textbf{$95\%$ CI} & \textbf{Best Baseline} & \textbf{$\Delta$} \\
\midrule
AquaSSM & AUROC & $0.916$ & $[0.934, 0.943]$ & MCN-LSTM $0.864$ & $+0.052$ \\
HydroViT v9 & $R^2$ (water temp) & $0.893$ & --- & DenseNet121 $0.884$ & $+0.009$ \\
MicroBiomeNet & macro-F$_1$ & $0.899$ & $[0.903, 0.923]$ & SimpleMLP $0.905$ & $-0.006$ \\
ToxiGene v2 & macro-F$_1$ & $0.877$ & $[0.857, 0.914]$ & SimpleMLP $0.890$ & $-0.013$ \\
BioMotion & AUROC & $1.000$ & $[1.000, 1.000]$ & DeepAutoenc.\ $0.958$ & $+0.042$ \\
\midrule
\textbf{Fusion (5-mod)} & \textbf{AUROC} & $\mathbf{0.992}$ & $[0.991, 0.993]$ & Sensor-only $0.943$ & $+0.049$ \\
\bottomrule
\end{tabular}
\end{table*}
...
\begin{table}[t]
\caption{Conformal prediction coverage at $\alpha\!=\!0.05$ (target $95\%$) by modality on real embeddings. Coverage is met only for the satellite channel with sufficient calibration volume.}
\label{tab:conformal}
\centering
\small
\begin{tabular}{lccc}
